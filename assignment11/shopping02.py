...
async def checkout_customer(queue: Queue, cashier_number: int):
    ...
    while not queue.empty():
        ...
        for product in customer.products:
            ...
            await asyncio.sleep(round(product.checkout_time))
        ...
        # cashier_take.update({'time': time.perf_counter() - customer_start_time})
        # cashier_take['time'] += round(time.perf_counter() - customer_start_time, ndigits=2)
        queue.task_done()
    cashier_take['time'] += round(time.perf_counter() - total_start_time, ndigits=2)
    return cashier_take
